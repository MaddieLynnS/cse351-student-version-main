...
words = ['BOOKMARK', 'SURNAME', 'RETHINKING', 'HEAVY', 'IRONCLAD', 'HAPPY', 
        ...
        'OVERCLOCKING', 'PRODUCER', 'DIFFUSE', 'SHORE', 'SOLD',
        'CELL', 'INDUSTRY', 'DIRT', 
    ...
    def _word_at_this_location(self, row, col, direction, word):
        ...
        for letter in word:
            ...
            if board_letter == letter:
                ...
                return False
        for _ in word:
            self.highlight(start_row, start_col)
            start_row += dir_x
            start_col += dir_y
        return True
...
def main():
    ...
    #     #results for this map is just true or false
    #     p.map(board.find_word, words)

    # processes = []
    # word_index = 0
    # while word_index < words.count:
    #     for _ in range(PROCESSES):
    #         p = mp.Process(target=board.find_word, args=(words[word_index]))
    #         word_index += 1
    #         p.start()
    #         processes.append(p)
